...
/*   Updated: 2024/05/13 12:52:09 by jasnguye         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	restore_redirections(t_data *data, int cmd_index, int io[2],
		int backup_fds[2])
{
	if (cmd_index > 0)
...
int	handle_redirections(t_data *data, int cmd_index, int *io_channels)
{
	...
	if (io_channels[1] != STDOUT_FILENO && dup2(io_channels[1],
			STDOUT_FILENO) == -1)
	{
